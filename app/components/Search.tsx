...
const Search = () => {
  ...
  const debouncedSearch = useDebouncedValue(query, 500); // Add debounce

  const { games, isLoading } = useGetGames({
    query: debouncedSearch,  // Use debounced search value
    isDisabled: debouncedSearch === "",
  });
  ...
  useEffect(() => {
    if (games) {
      ...
      console.log("Games not fetched yet.");
    }
  }, [games]);
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <XIcon
          onClick={() => {
            setQuery("");
            setActive(false); // Reset active state when clearing search
          }}
        />
        <SearchIcon className="w-5 h-5 cursor-pointer duration-150 group-hover:text-yellow-300" />
      </div>

      <AnimatePresence>
  {active && (isLoading || (games?.data?.results?.length > 0)) && (
    <MotionItem
      initial={{ height: 0 }}
      animate={{ height: "auto" }}
      exit={{ opacity: 0 }}
    ...
    >
      {isLoading ? (
        ...
        games?.data?.results.map((game: any) => (
          <div key={game.id} className="hover:bg-rose-500 duration-200 flex flex-col gap-2 px-4 py-2">
            <Link href={`/game/${game.id}`} className="flex gap-3 items-start w-full h-full">
              <div className="rounded-2xl relative overflow-hidden w-[40%] bg-neutral-900 h-20">
                <Image
                  ...
                  alt={game.name}
                  fill
                  sizes="(max-width: 768px) 100vw, 50vw"  // Add the sizes property
                  priority
                />
              </div>
        ...
        <p className="text-center text-white py-4">No games found with "{debouncedSearch}" query</p>
      )}
    </MotionItem>
  )}
</AnimatePresence>
